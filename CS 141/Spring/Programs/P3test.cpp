...
void playThree(char board[3],vector <string> &threeLetters, string randomWord, int &streakThree, int &timeThree, int &guesses){
  string word;
 // streakThree = 0;
  int attemptsThree = 0;
  ...
  int result = binarySearchString(word, threeLetters);
    ...
    if(word == randomWord){ //try board == random work next'
...
void playFive(char bigBoard[5],vector <string> &fiveLetters, string randomFive, int &streakFive, int &timeFive, int &guesses){
  string word;
  streakFive = 0;
  int attemptsFive = 0;
  ...
  int result = binarySearchString(word, fiveLetters);
    ...
    if(word == randomFive){ //try board == random work next'
      streakFive++;
      elapsedSeconds = difftime( time( NULL), startTime);
      cout << "Nice Work!  You guessed the correct word" << endl;
      cout << " - You completed the board in: "<< elapsedSeconds << " seconds." << endl;
...
int main(){
  ...
  int option;
  int streak = 1;
  int streakThree = 0; // number of win streak for three word
  int streakFive = 0; //number of win streak for five word
  ...
  cout << endl;
    if(option == 1){
      ...
      playThree(board,threeLetters,randomWord, streakThree, timeThree,guesses);
    }

    if(option == 2){
      randomFive = fiveLetters[rand() % fiveLetters.size()];
      playFive(bigBoard,fiveLetters,randomFive, streakFive, timeFive,guesses);

    }

    if(option == 3){
      streak = streakThree + streakFive; //  number of longest streak
      cout << "Overall Stats: "  << endl;
      cout << " - You guessed: "  << guesses<< endl;
      cout << " - Your longest streak is: " << streak<< endl;
